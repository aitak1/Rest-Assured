      ...
      try {
        ...
        nearbyLocations.forEach( (location) => {
          const marker = new google.maps.Marker({
                ...
                let maxRating = 5; // Maximum rating value
                let overallRating = (positive / total) * maxRating;
                ...
                {
                  infoWindow = new google.maps.InfoWindow({
                          content: `<div>${location.address}</div><div>Distance: ${location.distance} mi / ${(location.distance* 1.60934).toFixed(3)} km</div><div style="text-align: center; color: white; padding: 4px; background: ${color}">${(overallRating).toFixed(1)}/5</div>`
                        });
                }
